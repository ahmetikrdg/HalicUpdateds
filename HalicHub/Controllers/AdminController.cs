...
namespace HalicHub.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public IActionResult Email([FromBody]EMailModel m)  //Mail sınıfından m diye bir değişken tanımlarız
        {
            try
            {
        ...
        [AllowAnonymous]
        [HttpPost]
        public IActionResult EMailArticleDelete(string email)
        {
            var eml = _emailServices.GetByEmail(email);

            if (eml != null)
            {
                _emailServices.Delete(eml);
            }
